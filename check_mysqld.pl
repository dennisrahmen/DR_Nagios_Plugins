...
#  1. Specify these as command-line options when calling plugin, this is by far
...
#     This however has serious security problems as anyone who has login access to
#     nagios server can see user & pass in process when this plugin is being executed
...
for ($i=0;$i<scalar(@o_varsL);$i++) {
  if (defined($dataresults{$o_varsL[$i]}[0])) {
	...
	$dataresults{$o_varsL[$i]}[1]++;
	$statusdata .= " /n" . $o_varsL[$i] . "=" . $dataresults{$o_varsL[$i]}[0];
    }
